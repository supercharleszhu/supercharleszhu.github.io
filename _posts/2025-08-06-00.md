...
export TORCH_LOGS=+dynamo,+inductor,guard,recompile,aot
...
```
#### VariableTracker
[VariableTracker](https://github.com/pytorch/pytorch/blob/1de4540449ad6b9df8f452ab72da30ce8908af60/torch/_dynamo/variables/base.py#L235) is the base class for tracked locals and stack values
...
```
#### OutputGraph
[OutputGraph](https://github.com/pytorch/pytorch/blob/726dce3c944cbda16e54d3b15cdb4b6ced05af72/torch/_dynamo/output_graph.py#L378) is class located in InstructionTranslator to manage the generated FX graph. When FX graph is traced, it will call `compiles_and_call_fx_graph` to further compile it into hardware-specific executables (ex. Torchinductor generating cuda kernels)
...
When building up the graph, the VariableTracker generates comprehensive [guards](https://github.com/pytorch/pytorch/blob/ed77e23b6808065c927e626f471d5bf588dd2cc2/torch/_guards.py#L246) to make sure the pre-condition matches when we want to reuse an already compiled function
...
Guards are validated before we call a compiled function. It is implemented in cpp ([guard.cpp](https://github.com/pytorch/pytorch/blob/e025c0f45945eac2ece9091cc7c0c5f784b2b172/torch/csrc/dynamo/guards.cpp#L1704)) to provide better performance 
```cpp
...
See more details on how [guard](https://torchcompile-guards.hashnode.dev/inside-torchcompile-guards-how-they-work-what-they-cost-and-ways-to-optimize#heading-runtime-cost-how-to-reduce-guard-overhead) was implemented efficiently.
...
The `AOTAutograd` will basically help capturing backward graph if there are inputs that requires to capture gradient ahead of time (dispatcher [code](https://github.com/pytorch/pytorch/blob/25f1a5d8d13f404d7e08d64efce9aa6c411ee143/torch/_functorch/aot_autograd.py#L543)) (runtime [wrapper](https://github.com/pytorch/pytorch/blob/5b9114bf197d1db32cc1cfe06f66e2b4a02f55b1/torch/_functorch/_aot_autograd/runtime_wrappers.py#L2070) to generate final function with compiled fw and bw). 
...
See [how does autograd work for torch.compile](https://dev-discuss.pytorch.org/t/how-does-torch-compile-work-with-autograd/1621/2) for high-level ideas.
...
```
And internally it is running the fx graph and call fake mode of eager [autograd](https://github.com/pytorch/pytorch/blob/814338826e0b5cd065f8278c4b9487f13e16a5c7/torch/_functorch/_aot_autograd/graph_capture_wrappers.py#L396) (see details in [here](https://docs.pytorch.org/docs/stable/autograd.html))
...
```
Also, The Joint Graph (graph module) will go through several [passes](https://github.com/pytorch/pytorch/blob/7163dce1e091cb5564c723110314bb372b5e81a8/torch/_inductor/fx_passes/joint_graph.py#L561) for optimization. Ex. [remove_noop](https://github.com/pytorch/pytorch/blob/7163dce1e091cb5564c723110314bb372b5e81a8/torch/_inductor/fx_passes/post_grad.py#L1098), fuse_attention, etc. We can also register our own passes here.


Note: [Here](https://github.com/pytorch/pytorch/blob/ee53ad2dd0a864cd331241cd3c4e131da1ceb868/torch/fx/_utils.py#L9) is a great debug point to showcase the when the graph is generated;
Note: [API: functorch.compile.draw_graph(graph_module)](https://github.com/pytorch/pytorch/blob/167ad09be5af5c52666759412a3804068c6955d1/torch/_functorch/partitioners.py#L2890) can be used to draw visualized diagram of the graph 

The Joint fw+bw graph will be [partitioned](https://github.com/pytorch/pytorch/blob/167ad09be5af5c52666759412a3804068c6955d1/torch/_functorch/partitioners.py#L2683) into forward and backward graph separately via max-flow / min-cut algorithm, which is targeting at minimize the tensor being saved for backward passes. See more [details](https://dev-discuss.pytorch.org/t/min-cut-optimal-recomputation-i-e-activation-checkpointing-with-aotautograd/467) in this explanation. ([reference](https://en.wikipedia.org/wiki/Ford%E2%80%93Fulkerson_algorithm) of Ford–Fulkerson Algorithm)


#### Defined-by-run IR
After forward and backward graph is generated, it will generate a `lowered defined-by-run IR` for TorchInductor. These IR will have the layout
