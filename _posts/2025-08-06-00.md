---
title: PyTorch Compile Deep Dive - From Python Function to Triton Kernel
...
# PyTorch Compile Deep Dive: From Python Function to Triton Kernel
...
TorchDynamo is PyTorch's graph capture mechanism that converts Python functions into computational graphs. It operates by:

1. **Bytecode Analysis**: Dynamically analyzing Python bytecode
2. **Symbolic Execution**: Tracking tensor operations symbolically
3. **Graph Construction**: Building a computational graph representation

### 🔬 Deep Dive: TorchDynamo's Internal Architecture

Based on the PyTorch Dynamo codebase analysis, here's how TorchDynamo actually works under the hood:

#### **1. Entry Point and Frame Analysis**

The process starts in `convert_frame.py` when a function decorated with `@torch.compile` is called:

```python
# From convert_frame.py
def convert_frame_assert(compiler_fn, one_graph=True, export=False):
    def __call__(self, frame, cache_entry, hooks, frame_state, skip=0):
        # Frame Detection: Dynamo intercepts Python function calls
        # Bytecode Extraction: Extract function's bytecode using dis module
        # Initial Checks: Check if frame contains tensors and worth compiling
```

#### **2. Bytecode Analysis and Transformation**

TorchDynamo performs sophisticated bytecode analysis:
