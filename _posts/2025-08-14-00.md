...
A **tile** is just a block-sized chunk of a matrix that fits in shared memory or registers — a logical concept, not a hardware construct. In code, it's just a pointer + shape + stride into the full matrix:
...
The tile sizes (`kBlockM`, `kBlockN`) are not derived from a formula — they're chosen by **benchmarking** the tradeoff between several competing constraints:

- **Shared memory capacity**: The tile must fit. `smem ≈ (kBlockM * kHeadDim + 2 * kBlockN * kHeadDim) * 2 bytes`.
- **Occupancy**: Smaller tiles use less smem → more CTAs (thread blocks) per SM → better latency hiding.
- **Compute efficiency**: Larger tiles amortize softmax/masking overhead over more matmul FLOPs.
- **Causal masking waste**: With causal attention, roughly half the S tile is masked out. Square tiles (`kBlockM ≈ kBlockN`) waste less work.
...
The **Warps** column (`kNWarps`) controls how many warps (groups of 32 threads) execute per thread block. Larger head dimensions need more warps because the tiles are bigger:

- **More data to load**: The Q tile is `kBlockM x kHeadDim`. For hdim=128 that's 16K elements; for hdim=192 it's 24K. More warps means more threads issuing asynchronous `cp.async` loads in parallel.
- **More compute per tile**: The matmul `Q_tile @ K_tile^T` has `kBlockM x kBlockN x kHeadDim` FLOPs. More warps keep the Tensor Cores fed.
- **Shared memory pressure**: With hdim=192, smem usage is ~98KB — close to the per-SM limit, so only 1 CTA fits. But 8 warps (256 threads) are enough to saturate it.

These values are set at the launch site in the code (e.g., `Flash_fwd_kernel_traits<128, 128, 64, 4>` where `4` is `kNWarps`), and different GPU architectures get different configurations based on profiling — Tri Dao's comments in the code document the empirical findings:
...
This is a [CuTe](https://github.com/NVIDIA/cutlass/blob/main/media/docs/cute/00_quickstart.md) construct that describes how to map a large matrix multiply onto Tensor Cores. Let's break down the three parts:
...
So `_TN` means: A is **T**ransposed (column-major input read as row-major), B is **N**ormal (column-major). This matters for GPU performance because memory accesses are **coalesced** when adjacent threads read adjacent (stride-1) addresses.
