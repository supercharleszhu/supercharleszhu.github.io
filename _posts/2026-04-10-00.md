...
3. **NVLink** (hardware DMA at each step, zero CPU involvement)
4. **GPU memory** (reduced gradient, ready for optimizer)

No CPU memory, no serialization, no Python — just GPU kernels pushing data over NVLink. But the proxy thread on CPU must advance the state machine between steps, which is why CPU contention directly impacts AllReduce latency.
