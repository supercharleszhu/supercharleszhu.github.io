...
### 8-GPU Large Model Benchmark: Ray Train = torchrun

We scaled the test to 8 GPUs with a 1.2 GB model (SIZE=10000, three 10000x10000 linear layers, 100 MB DDP buckets) and 20 timed steps after warmup. We compared three approaches:

| Approach | Avg step time | Max step time | NCCL kernel | Protocol |
|----------|-------------|-------------|-------------|----------|
| **torchrun** (8 processes) | **5.96 ms** | 15.70 ms | AllReduce_Sum_f32_RING_LL | LL |
| **Ray Train** (8 actors x 1 GPU) | **5.95 ms** | 14.57 ms | AllReduce_Sum_f32_RING_LL | LL |
| **Ray mp.spawn** (1 actor x 8 GPU) | **6.04 ms** | 16.71 ms | AllReduce_Sum_f32_RING_LL | LL |

**All three are statistically identical** — ~6 ms/step with no measurable overhead from Ray Train actors. Even forcing `NCCL_PROTO=^LL,LL128` (which should disable LL) had no effect — NCCL still selected LL on H200 NVSwitch.

This is because:
1. On H200 with P2P/CUMEM, the LL protocol runs entirely on GPU — the CPU proxy is never on the data path (see next section)
2. At 8 GPUs with a 1.2 GB model, the GPU compute time per step (~6 ms) is large enough to hide any CPU-side jitter from Ray's background threads
3. The NCCL kernel itself (`RING_LL`) uses flag-based GPU-to-GPU signaling over NVLink — no CPU involvement at any ring step
...
### What This Means for Our 8-GPU Benchmark

On H200 with NVSwitch, all communication is intra-node P2P/CUMEM. The proxy does nothing. This explains why:

- **Ray Train = torchrun = mp.spawn** at 8 GPUs (~6 ms/step, identical)
- Even forcing `NCCL_PROTO=^LL,LL128` (to disable LL) had no effect — NCCL still chose LL because the P2P transport doesn't support Simple protocol for direct CUMEM connections
- The 2-GPU test showed Ray Train overhead not from proxy but from CPU jitter at DDP hook/kernel-enqueue boundaries, which is proportionally smaller with more GPU work per step
