...
This is why **large-model training (LLMs) uses Simple protocol where the CPU proxy IS on the critical path**, while small-model benchmarks like our ToyModel use LL where it isn't. The Ray Train background thread overhead would be more pronounced in large-model training.
...
From the trace: `ncclDevKernel_AllReduce_Sum_f32_RING_LL` — the **LL protocol**. This means the proxy thread was NOT on the data path. The 3x slowdown we measured in Ray Train vs torchrun is therefore NOT from proxy thread preemption during AllReduce.

#### Then Why Is Ray Train Slower?

If the LL protocol runs entirely on GPU with no CPU proxy involvement, where does the 3x overhead come from? The answer is that the CPU is still involved at the **boundaries**:

1. **`ncclAllReduce()` enqueue** (CPU): PyTorch calls `ncclAllReduce()` from `ProcessGroupNCCL::collective()`, which enqueues the kernel onto the CUDA stream. In Ray Train actors, this CPU call competes with background threads for CPU time.

2. **CUDA stream launch latency** (CPU → GPU): The CUDA driver must schedule the kernel onto the GPU. If the CPU thread calling `ncclAllReduce` is preempted between the API call and the driver processing the launch, the kernel sits in the stream queue longer.

3. **DDP bucket readiness** (CPU): The `Reducer::autograd_hook` → `mark_bucket_ready` → `all_reduce_bucket` chain runs on CPU. Background thread preemption here delays when the AllReduce is even enqueued.
...
- **Before/after AllReduce**: CPU is needed to enqueue the kernel, manage streams, and run DDP hooks. This is where Ray's background threads cause jitter.
...
Both `torchrun` and Ray Train (2 workers, 1 GPU each) showed identical NCCL behavior on this benchmark:
...
### When Is the Proxy Thread on the Data Path?

**It depends on the protocol.** The proxy progress function for P2P transport ([`p2p.cc:799-817`](https://github.com/NVIDIA/nccl/blob/master/src/transport/p2p.cc)) explicitly checks:
...
static ncclResult_t p2pSendProxyProgress(...) {
    ...
    for (int s = 0; s < args->nsubs; s++) {
        // ...
        if (p != NCCL_PROTO_SIMPLE) {  // ← KEY CHECK (line 817)
            resources->step = sub->base + sub->nsteps;
            args->done++;
            continue;  // Skip proxy work for LL and LL128 protocols!
        }
        // Only Simple protocol needs proxy to cudaMemcpy + cudaEventQuery
        // ...
        cudaMemcpyAsync(dst, src, size, cudaMemcpyDeviceToDevice, stream);
        cudaEventRecord(event, stream);
        // Poll: cudaEventQuery(event) — CPU must check if GPU copy completed
    }
}
```

| Protocol | Proxy on data path? | How signaling works |
|----------|-------------------|-------------------|
| **LL (Low Latency)** | **No** | GPU threads spin-poll flags in peer GPU memory via NVLink. Pure GPU. |
| **LL128** | **No** | Same as LL but with 128-byte packets. Pure GPU. |
| **Simple** | **Yes** | Proxy calls `cudaMemcpyAsync` + polls `cudaEventQuery`. CPU must advance. |

### LL Protocol: Entirely on GPU

For LL protocol ([`prims_ll.h`](https://github.com/NVIDIA/nccl/blob/master/src/device/prims_ll.h)), signaling between GPUs uses **flag-based polling entirely on GPU**:

```cpp
// prims_ll.h:57-60 — GPU thread waiting for send slot
inline __device__ void waitSend(int nbytes) {
    if (sendConnHeadPtr) {
        int spins = 0;
        while (sendConnHeadCache + NCCL_STEPS < sendConnHead + 1) {
            // Spin-poll peer GPU's head pointer via NVLink
            sendConnHeadCache = LOAD(sendConnHeadPtr);  // P2P/CUMEM read
        }
    }
}
```

Each LL FIFO line includes a 4-byte flag. The sender writes data + flag atomically. The receiver spin-polls the flag from GPU. No CPU involvement at any step.

### Simple Protocol: CPU Proxy Required

For Simple protocol, the proxy thread calls `cudaEventQuery()` in a busy-wait loop to track GPU completion ([`p2p.cc:822-847`](https://github.com/NVIDIA/nccl/blob/master/src/transport/p2p.cc)):

```cpp
// Check GPU has sent everything  
if ((*recvTail > sub->base + sub->transmitted)) {
    cudaMemcpyAsync(dst, src, size, cudaMemcpyDeviceToDevice, stream);
    cudaEventRecord(events[buffSlot], stream);
}
// Poll for completion
cudaError_t res = cudaEventQuery(events[buffSlot]);  // CPU → GPU query
if (res == cudaSuccess) {
    sub->done += args->sliceSteps;
    resources->shm->recvMem.tail = sub->base + sub->done;  // advance FIFO tail
}
```

The proxy runs with `CU_CTX_SCHED_SPIN` ([`proxy.cc:896`](https://github.com/NVIDIA/nccl/blob/master/src/proxy.cc)) — it busy-waits without sleeping:
...
Our benchmark showed `ncclDevKernel_AllReduce_Sum_f32_RING_LL` — the **LL protocol**. This means:

- The proxy thread was **NOT** on the AllReduce data path
- GPU threads handled all ring steps (send, reduce, recv) via direct NVLink P2P memory access
- No `cudaEventQuery` polling from CPU during the AllReduce

The 3x Ray Train slowdown we measured is NOT from proxy thread preemption during AllReduce. Instead, it comes from CPU contention at the **boundaries**:

1. **Kernel enqueue**: `ncclAllReduce()` CPU call competes with Ray background threads
2. **CUDA stream launch**: Driver must schedule the GPU kernel — CPU preemption delays this
3. **DDP hooks**: `Reducer::autograd_hook` → `mark_bucket_ready` → `all_reduce_bucket` chain runs on CPU
4. **Stream sync**: `ProcessGroupNCCL::collective()` calls `syncStream()` which involves CPU-side CUDA event ops

For **large-message training** (e.g., large model all-reduces with Simple protocol), the proxy thread WOULD be on the critical path, and CPU contention would directly stall the NCCL ring at every step.
